from numpy import zeros, array, conjugate, pi, round, savetxt, loadtxt
...
J = (1/2**0.5, 1/2**0.5, 1.0) # pure Kitaev terms 
H = (0.025/(3**0.5), 0.025/(3**0.5), 0.025/(3**0.5))   # magnetic terms 
...
for key in ansatz_terms_dict:
    ...
    result = minimize(fun=cost, x0=params0,  method=method, tol=0.0001, options={'maxiter':100}) # run optimizer
    ...
    #     result = minimize(fun=cost, x0=red_op_params,  method=method, tol=0.0001, options={'maxiter':20})
    #     nfev = nfev + result['nfev']
    #     nit = nit + result['nit']
    #     print(f"optimization success:{result['success']}")
...
savetxt('vqe_results/andy_lattice/h_025/params.txt', result['x'])
savetxt('vqe_results/andy_lattice/h_025/energy.txt', [optimal_energy])
